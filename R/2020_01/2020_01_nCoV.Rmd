...
map_df <- fortify(map) %>%
    ...
    mutate(contagion = case_when(
               is.na(updated) | updated > 1 & updated <= 9 ~ "0-9",
               updated > 10 & updated <= 99 ~ "10-99",
...
mapa_ncov <- ggplot(data = map_df, 
    ...
    geom_richtext(aes(x = 75, y = 15, 
                      label = "As of 8<sup>th</sup> of January 2020,<br>the bushfires have burnt<br>41,300 mi<sup>2</sup>"), 
                  family = "Oswald",
                  size = 3.7,
                  color = "black") +
